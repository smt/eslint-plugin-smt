...
module.exports = function ngAnnotateArgOrder (context) {
    return {
        ...
        AssignmentExpression: function assignmentExpressionRule (node) {
             ...
             * @var {String}
             */
            var patternEscaped = (patternPrefix + patternSuffix)
                                 .replace(escapeRE, "\\$&");

            /**
             * The escaped RegExp form of the key pattern.
             * @var {RegExp}
             */
            var patternRE = new RegExp(patternEscaped);
            ...
            if (source.match(patternRE) && node.right.type === 'FunctionExpression') {
